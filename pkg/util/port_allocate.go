...
func getClusterUsedNodePorts(client *kubernetes.Clientset) ([]int, error) {
	pods, err := client.CoreV1().Pods("").List(context.Background(), meta_v1.ListOptions{})
	if err != nil {
		return k8sClusterUsedPorts, err
	}
	...
	services, err := client.CoreV1().Services("").List(context.Background(), meta_v1.ListOptions{})
	if err != nil {
		return k8sClusterUsedPorts, err
	}
