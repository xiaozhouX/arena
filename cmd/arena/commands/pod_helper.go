...
func acquireAllActivePods(client *kubernetes.Clientset) ([]v1.Pod, error) {
	...
	nodeNonTerminatedPodsList, err := client.CoreV1().Pods(metav1.NamespaceAll).List(context.Background(), metav1.ListOptions{FieldSelector: fieldSelector.String()})
	if err != nil {
		return allPods, err
	}
...
func acquireAllJobs(client *kubernetes.Clientset) ([]batchv1.Job, error) {
	...
	jobList, err := client.BatchV1().Jobs(ns).List(context.Background(), metav1.ListOptions{})
	if err != nil {
		return allJobs, err
	}
